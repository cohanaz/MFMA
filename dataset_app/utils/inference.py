...
def plot_metrics_two_sets(
    ...
    table_data = [
        ['Accuracy', f"{metrics1[0]:.0%}", f"{metrics2[0]:.0%}"],
        ['AUC', f"{metrics1[1]:.0%}", f"{metrics2[1]:.0%}"],
        ['TPR@FPR=10%', f"{metrics1[2]:.0%}", f"{metrics2[2]:.0%}"],
    ]

    column_labels = ['Metric', labels[0], labels[1]]
    df = pd.DataFrame(table_data, columns=column_labels)
    return df
