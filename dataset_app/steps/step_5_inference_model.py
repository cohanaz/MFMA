...
from dataset_app.utils.inference import plot_metrics_two_sets, compute_tpr_at_fpr
...
def run():
    ...
    st.session_state.selected_feature_groups = selected_groups
    ...
    if "inference_model_trained" not in st.session_state:
        st.session_state.inference_model_trained = False
    ...
    if clicked:
        ...
        with col:
            ...
            # Train model
            with st.spinner(f"⏳ **Training inference model...**"):
            ...
            st.session_state.infrence_results = plot_metrics_two_sets(
            ...
            st.session_state.inference_model = clf
            st.session_state.inference_model_trained = True
            st.markdown("### 📊 Inference Model Results")
            st.dataframe(st.session_state.infrence_results, hide_index=True )
    ...
    with col1:
        if st.button("⬅ Back", use_container_width=True):
            ...
            st.rerun()
    with col2:
        if st.button("Next ➡", use_container_width=True, disabled=not st.session_state.inference_model_trained):
            st.session_state.active_step += 1
            st.rerun()